...
const CSV_HEADER = 'order_id,date,customer_name,sku_id,sku_name,category,supplier_country,quantity,unit_price,freight_cost,rebate_pct,warehouse,current_stock,avg_lead_days,order_status';
...
async function main() {
  ...
  if (anthropicKey) {
    ...
    const file = await (client.beta.files as any).upload(
      { file: new File([csvBlob as unknown as BlobPart], 'orders.csv', { type: 'text/plain' }) },
