  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        const outputFiles: { fileId: string; containerId: string; name: string }[] = [];
        ...
        for await (const event of response) {
          ...
          if (evType === 'response.output_item.done') {
            ...
            if (item?.type === 'code_interpreter_call') {
              const outputs = (item.outputs as Array<{ type: string; url?: string }>) ?? [];
              const containerId = (item.container_id as string) ?? '';
              for (const out of outputs) {
                if (out.type === 'image' && out.url) {
                  const m = out.url.match(/file-[a-zA-Z0-9]+/);
                  if (m) outputFiles.push({ fileId: m[0], containerId, name: 'output.png' });
                }
              }
            }
          }
        ...
        if (outputFiles.length > 0) {
          const meta = JSON.stringify({ outputFiles: outputFiles.map(f => ({ ...f, provider: 'openai' })) });
          controller.enqueue(encoder.encode(`8:${JSON.stringify([meta])}\n`));
        }

        controller.enqueue(encoder.encode(`d:{"finishReason":"stop","usage":{"promptTokens":0,"completionTokens":0}}\n`));
        console.log(JSON.stringify({ event: 'request_complete', panel: 'raw', provider: 'openai', total_ms: Date.now() - startMs }));
      } catch (err) {
...
async function handleAnthropic(model: string, prompt: string): Promise<Response> {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        const response = await client.beta.messages.create(
          {
            ...
            system: systemMsg,
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            tools: [{ type: 'code_execution_20250825', name: 'code_execution' } as any],
            messages: [{ role: 'user', content: userContent }],
            stream: true,
          },
          { headers: { 'anthropic-beta': 'files-api-2025-04-14,code-execution-20250825' } }
        );
        ...
        controller.enqueue(encoder.encode(`d:{"finishReason":"stop","usage":{"promptTokens":0,"completionTokens":0}}\n`));
        console.log(JSON.stringify({ event: 'request_complete', panel: 'raw', provider: 'anthropic', total_ms: Date.now() - startMs }));
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        controller.enqueue(encoder.encode(`3:${JSON.stringify(msg)}\n`));
...
export async function POST(req: Request) {
  ...
  console.log(JSON.stringify({ event: 'request_start', panel: 'raw', model, provider, timestamp: new Date().toISOString() }));
