        ...
        {messages.length === 0 && !error && (
          <div className="flex items-center justify-center h-full">
            <p className="text-sm text-center" style={{ color: "var(--text-secondary)" }}>
              ...
              <span className="text-xs" style={{ color: "var(--text-muted)" }}>or type your own question</span>
            </p>
          </div>
        )}
          ...
          <div key={msg.id} className="space-y-2">
            {msg.role === 'user' && (
              <div className="flex justify-end">
                <div className="bg-slate-800/60 text-slate-100 text-sm rounded-lg px-3 py-2 max-w-xs">{msg.content}</div>
              </div>
            )}
            {msg.role === 'assistant' && (
