...
export const tools = {
  get_customer_orders: tool({
    description:
      "Filter orders by customer name, returns list of orders with id, date, sku, qty, gross revenue",
    parameters: z.object({
      customer_name: z.string().describe("Customer name to filter by"),
    }),
  ...
  calculate_order_profitability: tool({
    description:
      "Calculate full profitability breakdown for a specific order including freight and rebate impact",
    parameters: z.object({
      order_id: z.string().describe("Order ID to calculate profitability for"),
    }),
    execute: async ({ order_id }) => {
      const order = ORDERS.find((o) => o.order_id === order_id);
      if (!order) return { error: `Order ${order_id} not found` };
      const { gross_revenue, rebate_amount, net_revenue, margin_pct } =
        calcProfitability(order);
      return {
  ...
  get_margin_distribution: tool({
    description:
      "Compute net margin distribution statistics across all orders using statistical analysis",
    parameters: z.object({}),
    execute: async () => {
      ...
      return {
        ...
        pct_unprofitable:
          Math.round((negativeCount / ORDERS.length) * 100 * 100) / 100,
      };
    },
  }),
  ...
  predict_reorder_urgency: tool({
    description:
      "Predict reorder urgency for a SKU using linear regression on order history to estimate daily burn rate",
    parameters: z.object({
      sku_id: z.string().describe("SKU ID to analyze (e.g. ZDR-006)"),
    }),
    execute: async ({ sku_id }) => {
      const skuOrders = ORDERS.filter((o) => o.sku_id === sku_id).sort((a, b) =>
        a.date.localeCompare(b.date),
      );
      if (skuOrders.length === 0)
        return { error: `No orders found for SKU ${sku_id}` };
      ...
      return {
        ...
        days_remaining,
        reorder_needed_by: new Date(
          Date.now() + (days_remaining - skuOrders[0].avg_lead_days) * 86400000,
        )
          .toISOString()
          .split("T")[0],
  ...
  get_supplier_margin_impact: tool({
    description:
      "Rank suppliers by average net margin impact — identifies which supplier relationships are most margin-destructive",
    parameters: z.object({}),
    execute: async () => {
      const bySupplier: Record<string, number[]> = {};
      ORDERS.forEach((o) => {
        const { margin_pct } = calcProfitability(o);
        if (!bySupplier[o.supplier_country])
          bySupplier[o.supplier_country] = [];
        bySupplier[o.supplier_country].push(margin_pct);
      });

      const ranked = Object.entries(bySupplier)
        .map(([country, margins]) => ({
  ...
  get_exception_orders: tool({
    description:
      "Return all orders where net margin falls below a threshold percentage",
    parameters: z.object({
      margin_threshold_pct: z
        .number()
        .describe(
          "Margin threshold percentage (e.g. 10 for orders below 10% margin)",
        ),
    }),
    execute: async ({ margin_threshold_pct }) => {
      const exceptions = ORDERS.map((o) => {
        const { gross_revenue, rebate_amount, net_revenue, margin_pct } =
          calcProfitability(o);
        return {
          order: o,
